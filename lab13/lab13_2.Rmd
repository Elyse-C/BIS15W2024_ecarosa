...
lat <- c(34.67, 41.80)
long <- c(-124.1, -115.5)
bbox <- make_bbox(long, lat, f = 0.03) #f is the fraction of the bounding box to add to the range. Makes it a little wider and a little longer to accommodate points at very edge of map. Always use a very small number.
```
...
ggmap(map1) + 
  ...
  labs(x= "Longitude", y= "Latitude", title="Shark Incident Locations")
```
...
```{r}
map1 <- get_stadiamap(bbox2, maptype = "stamen_terrain", zoom=7)
```

```{r}
ggmap(map1)+
  geom_point(data=sharks2, aes(longitude, latitude), size=0.4)+

...
```{r}
names(spider_list[[2]]) #must use double brackets
```
...
```{r}
left_exampleDF <- left_join(table_A, table_B, by="customer_ID") #data matched from left table (table A)
left_exampleDF
```
...
```{r}
right_exampleDF <- right_join(table_A, table_B, by="customer_ID") #data matched from right tale (table A)
right_exampleDF
```

`full_join`
```{r}
full_exampleDF <- full_join(table_A, table_B, by="customer_ID") #mashes everything together
full_exampleDF
```

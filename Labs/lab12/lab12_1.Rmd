...
#install.packages("RColorBrewer") #you don't want to knit without a hashtag
...
4. Use `fill` to show the range of `log10.mass` by taxon.
```{r}
homerange %>% 
...
homerange %>% 
  ggplot(aes(x = log10.mass)) +
  geom_histogram(color = "black", fill = "lavender", bins=10)+
  labs(title = "Distribution of Body Mass")
```
...
homerange %>% 
  ...
  geom_histogram(fill="lavender", alpha=0.8, color="black")+
...
homerange %>% 
  ggplot(aes(x=log10.hra))+
  geom_density(fill="lavender", alpha=0.6, color="black")+
  labs(title="Homerange",
       x="log10 Mass")
```
...
homerange %>% 
  ggplot(aes(x=log10.hra))+
  geom_histogram(aes(y=after_stat(density)), fill="lavender", alpha=0.8, color="black")+
...
#Know this for midterm 2. Will have to make categories like this for data that won't work unless you do this!!
```
...
homerange %>% 
  ...
  labs(title="Small Animal Homerange Range",
       x="Class",
       y="Log10 Mass",
       fill="Class")
```

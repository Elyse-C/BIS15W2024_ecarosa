...
ui <- fluidPage(titlePanel("Log 10 Homerange by Taxon"),
              ...
              choices=c("birds", "lake fishes", "river fishes", "marine fishes", "mammals", "lizards", "snakes", "turtles", "tortoises"),
              selected = "birds"),
    plotOutput("plot", width="600px", height="500px")
)
...
server <- function(input, output, session) {
  output$plot <- renderPlot({
    homerange %>% 
      filter(taxon==inputs$x) %>% 
      ggplot(aes(x=log10.hra))+
      geom_density(fill="maroon", color="black", alpha=0.6)
  })
}

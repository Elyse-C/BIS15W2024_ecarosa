...
```{r}
penguins %>% 
  filter(!is.na(body_mass_g)) %>%  # pull out all observations without a number
  group_by(island) %>% 
  summarize(mean_bodywt=mean(body_mass_g),
            total=n())
```
...
penguins %>%
  group_by(sex) %>% 
  summarize(across(contains("mm"), mean, na.rm=T)) #use this to correct the error
```

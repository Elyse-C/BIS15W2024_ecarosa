...
```{r}
life_history <- read_csv("data/mammal_lifehistories_v3.csv", na= c("NA", " ", ".", "-999", "not_measured")) %>% clean_names()
...
# makes replacemt of NA's specific to a variable
```
...
```{r}
city <- read_csv("data/cites.csv") %>% clean_names()
```

2. Use `naniar` to summarize the NA's in each variable.
```{r}
naniar::miss_var_summary(city)
```

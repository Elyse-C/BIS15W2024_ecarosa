...
elephants %>% 
  ...
  geom_point()+#two continuous variables: use a scatter plot
  geom_smooth(methods=lm, se=F) #line curves because I put "methods" instead of "method"
```
...
elephants %>% 
  ...
  labs(title="Elephant Age vs. Height", #adds a title
       x="Age (years)",
       y="Height (cm)")+
  theme(plot.title=element_text(size=rel(1.5), hjust = 0.5))#hjust moves the title from left(0) to right(1)
```
